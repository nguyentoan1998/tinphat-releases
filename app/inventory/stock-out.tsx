...
export default function StockOutScreen() {
    ...
    const loadFirst = async () => {
        try {
            setLoading(true);
            pageRef.current = 1;
            const res = await inventoryApi.getOutbound(1, PAGE_SIZE);
            setItems(res.data);
            setHasNext(res.hasNext);
            setTotal(res.total);
        } catch (e: any) {
            showDialog('Lỗi', e.response?.data?.message || e.message || 'Không thể tải xuất kho');
            setItems([]);
        } finally {
            setLoading(false);
            setRefreshing(false);
    ...
    const loadMore = useCallback(async () => {
        ...
        try {
            ...
            const res = await inventoryApi.getOutbound(nextPage, PAGE_SIZE);
            ...
            <View style={s.card}>
                <View style={s.cardInner}>
                    ...
                    <View style={s.cardTop}>
                        ...
                        <Text style={[s.statusText, { color: '#EF4444' }]}>Xuất kho</Text>
                    </View>
                    {/* Tên sản phẩm */}
                    {item.Product?.name && (
        ...
        <View style={s.root}>
            ...
            <SafeAreaView style={s.safe} edges={['top']}>
                ...
                <Animated.View entering={FadeInDown.duration(400)} style={s.header}>
                    ...
                    <View style={s.headerIcon}>
                        <LinearGradient colors={['#EF4444', '#F87171']} style={s.iconGrad}>
                            <PackageMinus size={20} color="#fff" strokeWidth={2} />
                        </LinearGradient>
                    </View>
                    <View style={{ flex: 1 }}>
                        <Text style={[s.title, { color: colors.textPrimary }]}>Xuất kho</Text>
                ...
                <Animated.View entering={FadeInDown.duration(400).delay(60)} style={s.summaryRow}>
                    <View style={[s.summaryCard, { backgroundColor: 'rgba(239,68,68,0.12)', borderColor: 'rgba(239,68,68,0.25)' }]}>
                        <Text style={[s.summaryVal, { color: '#EF4444' }]}>{total || items.length}</Text>
                        <Text style={[s.summaryLbl, { color: colors.textMuted }]}>Tổng phiếu</Text>
                    </View>
                    <View style={[s.summaryCard, { backgroundColor: 'rgba(248,113,113,0.12)', borderColor: 'rgba(248,113,113,0.25)' }]}>
                ...
                {loading ? (
                    ...
                    <View style={s.emptyW}>
                        <View style={s.emptyIcon}>
                            <PackageMinus size={36} color="#EF4444" strokeWidth={1.5} />
                        </View>
                        <Text style={[s.emptyTitle, { color: colors.textPrimary }]}>Chưa có phiếu xuất</Text>
                        <Text style={[s.emptyT, { color: colors.textMuted }]}>Dữ liệu xuất kho sẽ hiển thị ở đây</Text>
                    </View>
                ) : (
                    <FlatList
                        data={items}
...
const s = StyleSheet.create({
    ...
    idText: { fontSize: 11, fontWeight: FontWeights.bold, color: '#EF4444', letterSpacing: 0.5 },
