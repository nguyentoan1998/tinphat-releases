// Purchase Receipts Screen — Glassmorphism + View Modal + Search + Warehouse Filter
...
import { Package, ChevronLeft, RefreshCw, Calendar, Truck, X, ChevronRight, Hash, FileText, Search, Building2 } from 'lucide-react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { StatusBar } from 'expo-status-bar';
import { useRouter } from 'expo-router';

import { goodsReceiptApi, GoodsReceipt } from '@/lib/goods-receipt-api';
...
const STATUS_MAP: Record<string, { label: string; color: string; bg: string }> = {
    ...
    PENDING: { label: 'Chờ nhận', color: '#F59E0B', bg: 'rgba(245,158,11,0.15)' },
...
// ─── Detail Modal ─────────────────────────────────────────────────────────────
function ReceiptDetailModal({ receipt: r, onClose }: { receipt: GoodsReceipt; onClose: () => void }) {
    const { isDark } = useThemeStore();
    const colors = ThemeColors.light;
    const st = STATUS_MAP[r.status] ?? { label: r.status, color: '#94A3B8', bg: '' };

    const infoRows = [
        { Icon: Hash, label: 'Số phiếu', value: r.receiptNumber },
        { Icon: Truck, label: 'Nhà cung cấp', value: r.Supplier?.name || '—' },
        { Icon: Building2, label: 'Kho nhập', value: r.Warehouse?.name || '—' },
        { Icon: Calendar, label: 'Ngày nhận', value: fmtDate(r.receiptDate) },
        ...
        <Modal visible animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}>
            <View style={{ flex: 1 }}>
                ...
                <View style={[md.header, { borderBottomColor: colors.divider }]}>
                    <View>
                        <Text style={[md.title, { color: colors.textPrimary }]}>Chi tiết phiếu nhập</Text>
                        <Text style={[md.sub, { color: colors.textMuted }]}>{r.receiptNumber}</Text>
                    </View>
                ...
                <ScrollView contentContainerStyle={md.scroll} showsVerticalScrollIndicator={false}>
                    <Animated.View entering={FadeInDown.duration(400).springify().damping(18)} style={{ alignItems: 'center', paddingVertical: Spacing.xl, gap: 10 }}>
                        <LinearGradient colors={[st.color + '33', st.color + '11']} style={md.heroBg}>
                            <Package size={36} color={st.color} strokeWidth={1.5} />
                        </LinearGradient>
                    ...
                    <Animated.View entering={FadeInUp.duration(400).delay(100).springify().damping(18)}
                        ...
                        {infoRows.map(({ Icon, label, value }, i) => (
                            <View key={i} style={[md.infoRow, { borderBottomColor: colors.divider }]}>
                                <View style={[md.infoIcon, { backgroundColor: isDark ? 'rgba(139,92,246,0.15)' : 'rgba(139,92,246,0.08)' }]}>
                        ...
                        <Animated.View entering={FadeInUp.duration(400).delay(180).springify().damping(18)} style={{ marginBottom: Spacing.lg }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: Spacing.sm }}>
                                ...
                                <Text style={{ fontSize: FontSizes.sm, fontWeight: FontWeights.semibold, color: colors.textPrimary }}>
                                    Hàng nhập ({r.GoodsReceiptItem.length})
                                </Text>
                            </View>
                            {r.GoodsReceiptItem.map((item: any, i: number) => (
...
export default function PurchaseReceiptsScreen() {
    ...
    const load = async () => {
        try {
            setLoading(true);
            const [receiptRes, supRes] = await Promise.all([
                goodsReceiptApi.getReceipts().catch(() => []),
                supplierApi.getAll().catch(() => null),
            ]);
            const recList: GoodsReceipt[] = Array.isArray(receiptRes) ? receiptRes
                : (receiptRes as any)?.data ?? (receiptRes as any)?.items ?? [];
            setReceipts(recList);
            ...
            showDialog('Lỗi', e.response?.data?.message || 'Không thể tải phiếu nhập');
        } finally {
            setLoading(false);
            setRefreshing(false);
        }
    ...
    // Client-side filter: search + warehouse
    const filtered = useMemo(() => {
        ...
        return result;
    }, [receipts, supplierFilter, debouncedSearch]);

    const renderItem: ListRenderItem<GoodsReceipt> = useCallback(({ item: r, index: i }) => (
                ...
                <View style={[s.card, { borderColor: colors.cardBorder }]}>
                    ...
                    <View style={[s.cardInner, { backgroundColor: colors.cardBg }]}>
                        <View style={s.cardTop}>
                            <View style={[s.numBadge, { backgroundColor: isDark ? 'rgba(139,92,246,0.15)' : 'rgba(139,92,246,0.1)' }]}>
                                <Text style={[s.numText, { color: ACCENT }]} numberOfLines={1}>{r.receiptNumber}</Text>
                            </View>
                        ...
                        {r.GoodsReceiptItem && r.GoodsReceiptItem.length > 0 && (
                            <View style={s.infoRow}>
                                ...
                                <Text style={[s.infoText, { color: colors.textMuted }]}>{r.GoodsReceiptItem.length} mặt hàng</Text>
                            </View>
                        )}
                        <View style={s.cardBottom}>
                            <View style={s.infoRow}>
                                <Calendar size={13} color={colors.textMuted} />
                                <Text style={[s.infoText, { color: colors.textMuted }]}>{fmtDate(r.receiptDate)}</Text>
                            </View>
            ...
            <View style={{ flex: 1 }}>
                ...
                <SafeAreaView style={{ flex: 1 }} edges={['top']}>
                    ...
                    <Animated.View entering={FadeInDown.duration(400)} style={s.header}>
                        ...
                        <View style={{ flex: 1 }}>
                            <Text style={[s.titleT, { color: colors.textPrimary }]}>Phiếu nhập hàng</Text>
                            <Text style={[s.subT, { color: colors.textMuted }]}>{filtered.length}/{receipts.length} phiếu</Text>
                        </View>
                    ...
                    {/* Search bar */}
                    <Animated.View entering={FadeInDown.duration(400).delay(50)} style={[s.searchWrap, { backgroundColor: colors.inputBg, borderColor: colors.cardBorder }]}>
                        ...
                        <Animated.View entering={FadeInDown.duration(400).delay(80)}>
                            <ScrollView horizontal showsHorizontalScrollIndicator={false}
                                ...
                                <Pressable onPress={() => setSupplierFilter('all')}
                                    ...
                                    <Text style={[s.chipT, { color: supplierFilter === 'all' ? ACCENT : colors.textMuted, fontWeight: supplierFilter === 'all' ? FontWeights.semibold : FontWeights.regular }]}>Tất cả NCC</Text>
                                </Pressable>
                                {suppliers.map(sup => {
                                    ...
                                    return (
                                        <Pressable key={sup.id} onPress={() => setSupplierFilter(sup.id)}
                                            ...
                                            <Text style={[s.chipT, { color: active ? ACCENT : colors.textMuted, fontWeight: active ? FontWeights.semibold : FontWeights.regular }]}>{sup.name}</Text>
                                        </Pressable>
                                    );
                                })}
                            </ScrollView>
                    ...
                    {loading ? (
                        ...
                        <View style={{ alignItems: 'center', paddingVertical: 70, gap: Spacing.md }}>
                            <Package size={44} color={colors.textMuted} strokeWidth={1.5} />
                            <Text style={{ fontSize: FontSizes.base, color: colors.textMuted }}>
                                {search || supplierFilter !== 'all' ? 'Không tìm thấy phiếu nhập' : 'Không có phiếu nhập'}
                            </Text>
                        </View>
                    ) : (
                        <FlatList
