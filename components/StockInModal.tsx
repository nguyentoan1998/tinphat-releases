// StockInModal — đồng bộ style EmployeeFormModal
...
import { X, Warehouse as WarehouseIcon, Package, StickyNote, ArrowDownToLine, Save } from 'lucide-react-native';
import { Picker } from '@react-native-picker/picker';

import { inventoryApi, Product, Warehouse as WarehouseT } from '@/lib/inventory-api';
...
export default function StockInModal({ visible, onClose, onSuccess }: StockInModalProps) {
    ...
    useEffect(() => { if (visible) { loadData(); resetForm(); } }, [visible]);

    const resetForm = () => { setWarehouseId(''); setProductId(''); setQuantity(''); setNote(''); };
    ...
    const handleSubmit = async () => {
        if (!warehouseId) { showDialog('Lỗi', 'Vui lòng chọn kho nhập'); return; }
        ...
        try {
            setSubmitting(true);
            await inventoryApi.createStockMovement({ type: 'IN', warehouseId, productId, quantity: qNum, note: note || undefined });
            showDialog('Thành công', `Đã nhập ${qNum} sản phẩm vào kho`);
            handleClose();
            onSuccess();
        } catch (error: any) {
            const msg = error.response?.data?.message || error.message || 'Không thể nhập kho';
                ...
                <View style={s.overlay}>
                    ...
                    >
                        ...
                        <View style={[s.header, { borderBottomColor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.06)' }]}>
                            <ArrowDownToLine size={24} color="#38BDF8" />
                            <Text style={[s.headerTitle, { color: colors.textPrimary }]}>Nhập kho</Text>
                            <Pressable style={[s.closeBtn, { backgroundColor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.05)' }]} onPress={handleClose} disabled={submitting}>
                            ...
                            <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                                ...
                                <View style={s.field}>
                                    <View style={s.labelRow}>
                                        ...
                                        <Text style={[s.label, { color: colors.textSecondary }]}>Kho nhập *</Text>
                                    </View>
                                ...
                                <View style={s.field}>
                                    <View style={s.labelRow}>
                                        <Package size={14} color={colors.textMuted} />
                                        <Text style={[s.label, { color: colors.textSecondary }]}>Số lượng nhập *</Text>
                                    </View>
                        ...
                        <View style={[s.footer, { borderTopColor: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.06)' }]}>
                            <Pressable style={[s.saveBtn, { backgroundColor: '#38BDF8' }, submitting && s.saveBtnDisabled]} onPress={handleSubmit} disabled={submitting || loading}>
                                {submitting ? <ActivityIndicator size="small" color="#FFFFFF" /> : (
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                        <Save size={18} color="#FFFFFF" />
                                        <Text style={s.saveBtnText}>Xác nhận nhập kho</Text>
                                    </View>
                                )}
                            </Pressable>
